...
const { chunks, invertedIndex } = buildIndex();
...
console.log(`[index] 索引就绪: ${chunks.length} 个文档块, ${invertedIndex.size} 个索引词`);
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const contexts = retrieve(query.trim(), chunks, invertedIndex, 8);
